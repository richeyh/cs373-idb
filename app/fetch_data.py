...
for category in lists:
    ...
    for book in books:
        ...
        with app.app_context():
            author = Author.query.filter_by(Author.first_name==firstName).filter_by(Author.last_name==lastName).all()[0]
        if author:
            book_author = author
        else:
            ...
            with app.app_context():
                ...
                DB.session.commit()
        with app.app_context():
            q_book = Book.query.filter_by(Book.isbn==book_isbn).all()[0]
        if q_book:
            b = q_book
